...
function exactMatchTimestamp(raw: WtaMatch | null | undefined): number {
  ...
  for (const key of ["MatchTimeStamp", "scheduledTime", "scheduled_time", "matchDate", "match_date", "date"]) {
    ...
    if (typeof value === "string" && value && /T\d{2}:|\d{2}:\d{2}/.test(value)) {
...
function recentSingles(matches: SupabaseMatch[]): DashboardData["recentSingles"] {
  return [...matches]
    ...
    .map((m) => {
      ...
      const ealaIs1 = String(raw.player_1) === String(EALA_ID);
      const opponent =
        raw.opponent && typeof raw.opponent === "object"
          ? String((raw.opponent as Record<string, unknown>).fullName ?? "Opponent")
          : String(ealaIs1 ? raw.team_name_2 ?? "Opponent" : raw.team_name_1 ?? "Opponent");
      return {
        result: winner === (ealaIs1 ? 1 : 2) ? "W" as const : "L" as const,
        opponent,
        tournament: String(raw.TournamentName ?? "Tournament"),
        date: m.match_date
          ? new Date(m.match_date + "T00:00:00Z").toLocaleDateString("en-GB", {day:"numeric",month:"short",year:"numeric"})
          : "—",
        round: String(raw.round_name ?? "—"),
        score: recentScore(raw),
      };
    });
}
  ...
  try {
    ...
    const [matches, statsRows, seasonRows, rankings, nextRows] = await Promise.all([
      fetchTable<SupabaseMatch>(
        `eala_matches?player_id=eq.${EALA_ID}&category=eq.singles&status=eq.completed&select=match_date,match_start,round_name,eala_won,raw_json&limit=500`
      ),
      fetchTable<SupabaseStats>(
        `eala_stats?player_id=eq.${EALA_ID}&select=updated_at,highest_singles_ranking,highest_doubles_ranking,grand_slam_singles&limit=1`
      ),
      fetchTable<SupabaseSeasonStats>(
        `eala_season_stats?player_id=eq.${EALA_ID}&season_year=eq.${displaySeason}&select=season_year,updated_at,singles_wins,singles_losses,doubles_wins,doubles_losses,singles_titles,doubles_titles,grand_slam_singles&limit=1`
      ),
    ...
    if (!stats || !season) throw new Error("Eala stats are not populated");
    ...
    return {
      lastUpdated: season.updated_at ?? stats.updated_at ?? null,
      ...
      doublesRank,
      singlesRecord: {
        wins: season.singles_wins ?? 0,
        losses: season.singles_losses ?? 0,
      },
      doublesRecord: {
        wins: season.doubles_wins ?? 0,
        losses: season.doubles_losses ?? 0,
      },
      singlesTitles: season.singles_titles ?? 0,
      doublesTitles: season.doubles_titles ?? 0,
